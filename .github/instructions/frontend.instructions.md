...
**Boolean prop naming convention:** Boolean `@Prop()` declarations must use single descriptive adjectives — no `is`, `has`, or `show` prefix. Follow the same style as native HTML boolean attributes (`disabled`, `required`, `open`, `controls`). Examples: `hasHeader` → `header`; `showClose` → `closable`; `hasClear` → `clearable`; `isError` → `error`. This rule applies to public `@Prop()` only — internal `@State()` names such as `isVisible` or `isDisabled` are not affected.
