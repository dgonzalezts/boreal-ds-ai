...
Also confirmed while auditing `bds-date-picker`'s `calendarType="default"` mode (EOA-17138 Task 15d): `border-radius: 2px` on the same host is the correct baseline (small, easy to mistake for "no radius" if eyeballing a screenshot) — it matched identically between `basic` and `default` modes, along with `background-color`, `padding` (12px 24px on `.popover-header`/`.popover-content`), and `filter`. A per-`calendarType` day-grid cell styling audit is largely redundant by construction, since every mode renders through the same shared `bds-calendar-grid` component instance — the real risk area for a "no header/footer chrome" mode is the *outer container's* corner-radius/shadow surviving with no header/footer siblings to anchor against, which this technique verifies directly.

**How to apply:** when a QA task asks to confirm popover container styling (radius/shadow) is unaffected by a structural change (e.g. removing header/footer chrome), pull `getComputedStyle` values for `border-radius`, `filter`, `background-color`, and `padding` on the `bds-popover` element directly (e.g. via `playwright-cli eval`), and diff them against a known-good reference scenario in the same page — don't rely on screenshot comparison alone for this class of check.
