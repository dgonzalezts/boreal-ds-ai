...
- **Styling:** Global CSS and design token stylesheets apply directly to component internals without needing `::part()` selectors or CSS custom properties to cross a Shadow Root boundary. **The `:host` pseudo-class does not work** — it requires a shadow boundary (see [MDN](https://developer.mozilla.org/en-US/docs/Web/CSS/:host)). Use direct tag name selectors instead: `bds-button { ... }`, `bds-checkbox:focus-visible { ... }`, etc.
...
- **BEEQ difference:** The BEEQ reference implementation (`.ai/lib/endava-beeq.txt`) uses Shadow DOM. Any BEEQ patterns that involve `::part()`, CSS custom property tunnelling, `:host` selectors, or `ElementInternals` shadow-mode behavior do not apply directly — they must be adapted for light DOM before adoption.
