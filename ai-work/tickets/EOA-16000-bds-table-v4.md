...
- **Not yet resolved — every item in this ticket needs its own design/scoping pass before task-level planning can begin**, mirroring how the column footer and responsive toolbar each needed a sign-off gate in `EOA-15507`. In particular:
  ...
  - Custom cell content via `<template slot="cell">` should reuse the row-identity content cache added when fixing the `formatter` re-render flash bug (see `bds-table.tsx`'s `_formatterNodes` map / `applyCellFormatter`, keyed by `` `${colKey}:${rowId}` `` and guarded by row-object identity): generalize it into a shared cache both `formatter` and `<template>` write into, rather than building a second parallel cache. The identity guard (`cached.row === row`) already gives correct behavior under virtualized row recycling — a recycled `<tr>`/`<td>` backed by a different row reference naturally misses the cache and re-clones — so this does not need to be re-derived as a new design problem during scoping. Suggest sequencing this task as "extract shared cache, then build `<template>` on top" rather than two independent implementations, to avoid duplicating coverage in the ticket's final combined mutation-testing pass.
  - **Right-edge column pinning is not yet on this ticket's backlog and needs its own scoping pass before it's added as a task.** Today's pinning is left-only by design: `pinnedColKeys` is a single `Set`, `ITableColumn.pinnable` is a plain boolean with no direction concept, and `updatePinnedColumnOffsets` only ever computes `style.left` by walking pinned `<th>`s in left-to-right DOM order — there is no `style.right` branch anywhere, and pinning the last column today just stacks it after other left-pinned columns rather than anchoring it to the right edge. Other table libraries (e.g. Nuxt UI, `simple-table.com`) model this as a per-column direction enum (`pinned: 'left' | 'right'`) with a mirrored right-anchored offset calculation and a divider flipped to the left edge of the right-pinned group. Implementing it needs: a new pin-direction prop (replacing or extending the boolean `pinnable`), a parallel right-offset computation alongside `updatePinnedColumnOffsets`, mirrored CSS, and — same as grouping/resizing above — a new interaction design for choosing pin direction, since today's single pin-icon toggle is boolean-only.
