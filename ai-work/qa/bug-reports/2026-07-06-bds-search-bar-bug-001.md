...
Recommend option 1 immediately (cheap, removes a misleading claim) and treating option 2 as a separate, scoped follow-up ticket if any real consumer is expected to pass lists in the hundreds-to-thousands range — not clear that one currently exists in this codebase.
