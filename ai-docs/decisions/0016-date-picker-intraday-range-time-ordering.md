...
`expanded + range + with-time` already correctly commits a genuine intraday range (same calendar day, two distinct times) when reached via preset-then-manual-edit: `rangeEndShiftApplies` skips ADR 0015's coverage shift once `selectedPreset === CUSTOM` (which any manual time edit already triggers), and the commit path combines each bound independently via `combineDateTimeToUTC`. Nothing downstream special-cases `rangeStart === rangeEnd`.
...
- **A second, narrower quirk is introduced, on top of ADR 0015's own.** For the specific case of a same-day `expanded` selection with an inverted end time, the header (and, per the 2026-09-22 revision above, the grid) shows a resolved boundary one day past the real day the user clicked — analogous to, but distinct from, ADR 0015's coverage-shift quirk (which applies to presets and `basic` manual selection, day-granularity, always +1 day, and never touches the grid). This one applies only to a manually-inverted same-day `expanded` pair, and must be documented alongside ADR 0015's existing quirk in `bds-date-picker.mdx`, not conflated with it — they have different triggers and, now, different grid behavior, even though both resolve via the same "+1 day, shown explicitly" mechanism.
