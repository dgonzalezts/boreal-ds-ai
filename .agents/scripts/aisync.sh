...
#
# Usage: aisync.sh [repo-root]
#   repo-root  Path to the repository root (defaults to the root of the current
#              git repository).
...
if git -C "$wt" diff --cached --quiet; then
  ...
  git -C "$wt" commit -m "sync: update AI configuration $(date '+%Y-%m-%d %H:%M')"
