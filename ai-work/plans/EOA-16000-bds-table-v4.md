...
**Status:** ✅ done — implementation, unit tests (`bds-table.reorder.spec.ts`, 2595/2595 suite-wide passing), docs (`ReorderableColumns` story, `Column reorder` anatomy section incl. the grouped-column exclusion note), and manual QA (web components + React + Vue parity) all verified. Three real bugs were found and fixed along the way, none caught by unit tests alone: (1) `moveColumn`'s "insert before target" semantics made dropping a column onto its immediate right neighbor a silent no-op — fixed with a direction-aware insert; (2) unkeyed `<th>` elements let keyboard focus desync from column identity after a reorder, causing `ArrowLeft`/`ArrowRight` to oscillate instead of progress — fixed via `key={col.colKey}` plus an explicit `componentDidRender` refocus (moving a keyed node via `insertBefore` blurs it even as the same DOM object, a genuine browser quirk, not a Stencil bug); (3) `updatePinnedColumnOffsets`'s dirty-check didn't track `columnOrder`, leaving a pinned column's header offset stale after any reorder that didn't touch it directly — fixed by adding it to the dirty-check. A follow-up design fix relocated the six-dot handle from the right (grouped with sort/pin) to the left of the label, matching the `bds-card-header` precedent this feature was modeled on — that change briefly broke the table's visual layout entirely by setting `display: flex` directly on `<th>` (overriding its `table-cell` role), caught by manual visual inspection (not any subagent check) and fixed by moving the flex layout to a new inner wrapper span instead.
...
**Status:** ✅ done — implementation, unit tests (`bds-table.resize.spec.ts`, 239/239 suites, 2615/2615 tests suite-wide passing), docs (`ResizableColumns`/`ResizableColumnsVirtualized` stories, `Column resizing` MDX section), and manual QA (web components + React + Vue parity) all verified. Three real bugs were found and fixed mid-task: (1) `currentColumnWidthPx` measured the `<col>` element's `getBoundingClientRect()`, which generates no rendered box per CSS 2.1 §17.2.1 and returns a zero-size rect in real browsers (invisible to jsdom, which zeroes every element) — fixed by preferring the parsed `effectiveColumnWidth()` over a DOM measurement; (2) `table { width: 100%; table-layout: fixed }` with all columns at explicit widths summing to less than the table's rendered width caused the browser to proportionally re-expand columns, fighting any JS-driven shrink — fixed via `allColumnsHaveExplicitWidth` + `syncNaturalTableWidth()`, which sets an explicit `calc(...)` table width from the live column widths instead of relying on `100%`; (3) `handleResizeHomeKey` left `aria-valuenow` stale after a reset — fixed by setting the attribute directly. A fourth suspected bug (React/Vue-only first-keyboard-resize no-op) was investigated twice and never reproduced against a correctly built package — both times the live `pnpm dev` dev server was serving a stale `boreal-react`/`boreal-web-components` bundle (Vite's dependency pre-bundling cache doesn't invalidate on a linked workspace package's dist output changing), which is what actually produced the symptom. Confirmed via 4 clean-build reproduction attempts (`pnpm dev:pack:react`/`dev:pack:vue`, which bypass the cache via a real tarball reinstall) with zero repros — no code change was needed. Lesson for future framework-wrapper QA on this project: verify React/Vue-specific behavior via `dev:pack:react`/`dev:pack:vue`, not the live dev server, whenever a symptom is framework-specific and doesn't reproduce in the raw web-component playground. Post-verification docs follow-up: added the undocumented `40px` resize floor to the "Column resizing" MDX section, and consolidated all keyboard documentation (which had drifted — "Column resizing" had grown its own duplicate `KeyboardDocs` table instead of following the "Column reorder" precedent of prose + a single canonical table in "Accessibility") into the "Accessibility" section, which also gained a previously-missing semantic-markup bullet for the resize handle's `role="separator"`/`aria-orientation`/`aria-valuenow`. A research doc explaining the full feature in plain terms lives at `ai-work/research/2026-08-03-bds-table-column-resizing-explained.md`.
...
**Status:** ✅ done — implementation (`lastSelectedRowId` anchor state, `computeRangeRowIds` pure util, capture-phase `handleCheckboxShiftCapture` on `click`/`keydown`/`mousedown` to work around `bds-checkbox`'s `bdsChange` not carrying the originating event), unit tests (13 new tests across `bds-table.selection.spec.ts`/`bds-table.utils.spec.ts`, plus 6 more regression tests below — 239/239 suites, 2655/2655 tests suite-wide passing), docs (`WithSelection` extended, new `WithCrossPageRangeSelection` story, new "Shift+range selection" MDX subsection, and the "Current limitations" table's now-resolved shift-range row removed), and manual QA (web components + React + Vue parity, via `dev:pack:react`/`dev:pack:vue`) all verified. Three real bugs were found (two via manual QA, one via manual testing after QA had already signed off) and fixed: (1) shift+click is also the browser's native text-selection-extend gesture — without `preventDefault()`, the original implementation both range-selected rows AND highlighted all cell text between the two click points; fixed by extending `handleCheckboxShiftCapture` to suppress the default action on shift+`mousedown` over a checkbox cell. (2) A pre-existing, unrelated bug surfaced during QA and bundled into this task per explicit decision: `onRowsChange`/`setupRowsPagination` read the slotted `bds-pagination`'s `itemsPerPage` property synchronously, before Stencil finishes hydrating that element and reflecting its `items-per-page` attribute onto the live property — silently mispaginating to the class default (`10`) whenever `.rows` is set before that hydration completes. This wasn't caught earlier because it never manifested in this plan's own manual-test playgrounds (a follow-up live repro attempt against plain static markup + a trailing `<script>` did NOT reproduce it either — HTML parsing performs a microtask checkpoint after each script block, so an already-defined `bds-pagination` element's microtask-based hydration chain typically finishes before the next script runs; the race needs a genuine macrotask gap, e.g. a first-ever lazy chunk load or a framework wrapper deferring the `.rows` assignment to a later tick). Fixed defensively via `getEffectiveItemsPerPage()`, narrowed after review to only fall back to the raw attribute while the property still sits at the un-hydrated default, otherwise always trusting the live (validated) property — an earlier version bypassed `bds-pagination`'s own value normalization permanently, which was caught and corrected before landing. This fix is a scoped workaround, not the principled fix (`componentOnReady()`-based); see the follow-up note after the acceptance criteria below. (3) After manual QA had already passed all checklist items, Shift+Space was found to do nothing at all — root cause was in `bds-checkbox` itself (`bds-checkbox.tsx`, `componentDidLoad`), not `bds-table`: its `KeyboardController` binding registered plain `Space` with no modifiers, and the controller does exact-modifier-match lookup, so Shift+Space (`shift+' '`) never matched and `toggle()`/`bdsChange` silently never fired. Undetected until now because every prior test either dispatched `bdsChange` synthetically or only exercised mouse clicks — nothing drove a real Shift+Space keydown through the actual keyboard controller. Fixed by also registering `[KEYBOARD.Space, KEYBOARD_MODIFIERS.Shift]` for the same toggle handler; covered by new `bds-checkbox` keyboard tests plus a `bds-table` end-to-end test that drives the real keydown path (not synthetic `bdsChange`). The manual-test playgrounds (`index.html`, `App.tsx`, `App.vue`) had no keyboard-only shift-range step either — added one to all three so future QA passes actually exercise this path.

**Follow-up finding (not a bug):** the user asked whether cross-page range selection was tested against `selectAllPages`. It wasn't, and it surfaced a real but *intentional* interaction worth documenting: with `selectAllPages` at its default (`false`), toggling the header "select all" checkbox on one page can never see or clear rows selected — via range or individual clicks — on a different page, since `selectAllScope` (`bds-table.tsx:710-714`) only ever resolves to the current page when `selectAllPages` is `false`. This matches the prop's own pre-existing documented contract from Task 8 and is not new to this task. Added a regression test proving off-page range-selected rows survive a same-page select-all/deselect-all toggle, and a `<Callout>` in the "Shift+range selection" MDX subsection clarifying the interaction so it isn't mistaken for a bug again.
...
**Follow-up (deferred, not this task):** while fixing a QA-found bug where `bds-table` read `bds-pagination`'s `itemsPerPage` before it finished hydrating, the fix landed (`getEffectiveItemsPerPage()` in `bds-table.tsx`) is a scoped workaround, not the principled fix — it falls back to the raw `items-per-page` attribute only while the live property still equals `bds-pagination`'s un-hydrated class default (`10`), which is a heuristic, not a real hydration signal. The correct fix is to `await paginationEl.componentOnReady()` before the first read, which always returns the validated/normalized value with no guessing — but `onRowsChange`/`setupRowsPagination` are currently synchronous Stencil watchers, so making that change properly requires reworking the initial pagination-setup path to tolerate an async first slice (either a brief render frame at the un-hydrated size, or restructuring the setup flow). Worth a dedicated follow-up task rather than folding into this one.
