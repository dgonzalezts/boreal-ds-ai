...
- [ ] `bds-table` supports `virtual` opt-in row virtualization backed by the already-present `@tanstack/virtual-core` dependency
...
- Whether `VirtualScrollController` should eventually be generalized to support true DOM-count virtualization (not just positional) given it now has two candidate consumers (`bds-search-bar`'s existing use, and the pattern `bds-table` needs) — not resolved here; `bds-table` builds its own integration against `@tanstack/virtual-core` directly for now.
