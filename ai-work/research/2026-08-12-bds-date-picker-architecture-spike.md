...
### Unscheduled — `labels` prop pattern inconsistency (design-system-wide, new, 2026-09-08)

- **What it is**: `bds-date-picker` is the only component in the library that localizes its own UI copy through a single bundled-object prop (`@Prop() labels?: DatePickerFooterLabels`, currently covering `clean`/`cancel`/`apply`/`hour`/`minute`/`start`/`end`, merged against an English default via `{ ...DEFAULT_FOOTER_LABELS, ...labels }`). Confirmed via a repo-wide `labels?:` grep across every component in `packages/boreal-web-components/src/components/`: exactly one hit (`bds-date-picker.tsx`). Every other localizable component uses one flat string `@Prop()` per label instead, each with its own English default — `bds-search-bar.searchButtonLabel`, `bds-table.selectionLabel`, `bds-pagination.perPageLabel`, `bds-banner.closeButtonLabel`, `bds-tag.closeButtonLabel`. `bds-date-picker` diverged from this when Clean/Cancel/Apply were first added (v1) and nothing has reconciled it since; `start`/`end` (v2, Task 19) and any future Phase 6/7 label additions (presets list item text, banner title/message defaults, footer range-summary phrasing) keep extending the same one-off shape rather than triggering a reconsideration.
- **Separately, "full" localization has gaps independent of which prop shape wins**: `locale` (`DateEngineLocale`, a plain `date-fns` `Locale` alias — `date-engine/types.ts`) and `labels` are two fully disconnected systems today — `locale` drives date-fns formatting (weekday/month names) only, never UI copy. No pluralization support exists anywhere (Phase 7's footer summary needs "1 day" vs. "18 days," and a flat string prop can't express plural rules). No project-wide i18n/message-catalog mechanism exists at all — confirmed via repo-wide search for "i18n"/"translation"/"locale-catalog"/"ICU" and a check of `.agents/memory/`/`ai-docs/` (no hits beyond incidental references in unrelated external-library reference docs). A consumer must pass every string individually per instance today, with no way to supply "the Spanish set" once.
- **Not a blocker for v3's Phase 5-9 work**: Task 23 (Phase 5 time-selector `Start:`/`End:` labels) only needs a new call site consuming the `start`/`end` keys v2's Task 19 already added — no new prop, no pattern change, low-risk regardless of this finding. Phase 6 (presets list text) and Phase 7 (banner/footer-summary text) will each add more strings under the current bundled-object shape unless this is deliberately reconsidered first — worth a call before those phases start, not mid-task.
- **Decision needed, deferred**: either conform `bds-date-picker` to the flat-per-prop convention (breaking its own precedent, but matching the other five components), or adopt the bundled-object approach library-wide going forward (the reverse — arguably better justified now, since `bds-date-picker` already has 7 keys and growing, versus one-off single strings elsewhere). Pluralization and a locale-catalog mechanism are a larger, separate question than either prop-shape choice. Recommend a short ADR or a dedicated ticket, not a decision folded silently into a Phase 6/7 task's acceptance criteria.
- **Resolved (2026-09-08)**: see [ADR 0014](../../ai-docs/decisions/0014-localizable-ui-copy-prop-shape.md) — threshold rule adopted (1 string → flat prop, 2+ → bundled-object prop). `bds-date-picker` keeps `labels`/`DatePickerFooterLabels`; Phase 6/7 extend it directly, no further shape decision needed. Pluralization and a locale-catalog mechanism remain open, tracked as non-goals in the ADR.
