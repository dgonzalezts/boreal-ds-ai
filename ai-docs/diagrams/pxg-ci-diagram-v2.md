# CI Diagram v2 — Boreal DS

## Changes from v1

- All `npm ci` → `pnpm install --frozen-lockfile`
- All `npm run` → `pnpm` (Turborepo-aware commands)
- Job 1: tests now use `pnpm test` (Turborepo), coverage via `pnpm test:coverage`
- Job 1: added `changeset status` check to enforce changeset presence on PRs touching publishable packages
- Job 1c: `npm audit` → `pnpm audit`
- Job 2: `npm run cem` → `pnpm --filter @boreal-ds/web-components cem`
- Job 2: `npm run build` → `pnpm build` (runs via Turborepo across all packages)
- Job 3: `npm run build-storybook` → `pnpm --filter @boreal-ds/docs build`
...
flowchart TD
  ...
  J1_Checkout --> J1_Install[Install Dependencies<br/>pnpm install --frozen-lockfile]
  J1_Install --> J1_Changeset[Check Changeset<br/>pnpm changeset status --since=release/current<br/>Fail if publishable pkg changed<br/>with no changeset filed]
  J1_Changeset --> J1_Lint[Run ESLint<br/>pnpm lint]
  J1_Lint --> J1_Test[Run Unit Tests<br/>pnpm test]
  J1_Test --> J1_Coverage[Check Test Coverage<br/>pnpm test:coverage<br/>Minimum 80%]
  J1_Coverage --> J1_Archive[Archive Test Results<br/>JUnit XML]
  J1_Archive --> J1_End[Job 1 Complete ✓]
  J1_Changeset -.No Changeset Found.-> NotifyFail1[❌ Notify Team<br/>Email + Teams]
  ...
  J1b_Checkout --> J1b_Install[Install Dependencies<br/>pnpm install --frozen-lockfile]
  ...
  J1c_Checkout --> J1c_Install[Install Dependencies<br/>pnpm install --frozen-lockfile]
  J1c_Install --> J1c_Audit[Run Security Audit<br/>pnpm audit<br/>Check Known Vulnerabilities]
  ...
  J2_Checkout --> J2_Install[Install Dependencies<br/>pnpm install --frozen-lockfile]
  J2_Install --> J2_CEM[Generate CEM<br/>pnpm --filter @boreal-ds/web-components cem<br/>Create custom-elements.json]
  J2_CEM --> J2_Build[Build Components<br/>pnpm build<br/>Turborepo — all packages in order]
  J2_Build --> J2_Validate[Validate Build Outputs<br/>• ESM Bundle<br/>• CJS Bundle<br/>• Type Definitions]
  J2_Validate --> J2_Archive[Archive Library Artifacts<br/>dist/ folders]
  ...
  J3_Checkout --> J3_Install[Install Dependencies<br/>pnpm install --frozen-lockfile]
  J3_Install --> J3_Build[Build Storybook<br/>pnpm --filter @boreal-ds/docs build]
  ...
  class J1_Checkout,J1_Install,J1_Changeset,J1_Lint,J1_Test,J1_Coverage,J1_Archive ciJob
  class J1b_Checkout,J1b_Install,J1b_Sonar,J1b_Gate,J1b_Archive ciJob
  class J2_Checkout,J2_Install,J2_CEM,J2_Build,J2_Validate,J2_Archive ciJob
