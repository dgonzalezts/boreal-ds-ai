...
- **Not yet resolved — every item in this ticket needs its own design/scoping pass before task-level planning can begin**, mirroring how the column footer and responsive toolbar each needed a sign-off gate in `EOA-15507`. In particular:
  ...
  - **Resolved (Task 7):** right-edge column pinning shipped as an additive `@Prop() readonly pinDirection: 'left' | 'right' = 'left'` on `bds-table-column`, read only when `pinnable` is `true` — rather than replacing the boolean `pinnable` with a direction enum. `updatePinnedColumnOffsetsRight()` mirrors `updatePinnedColumnOffsets()`, walking `th[data-pinned-right]` right-to-left and writing `style.right`; the right-pinned group's divider renders on its own leftmost cell via a new `data-pin-first` attribute, mirroring the left-pinned group's `data-pin-last`. Existing `pinnable` columns with no `pinDirection` are unaffected. See `bds-table.mdx`'s [Column pinning](#column-pinning) section.
