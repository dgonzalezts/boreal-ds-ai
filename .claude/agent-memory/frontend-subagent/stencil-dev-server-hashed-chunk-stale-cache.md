...
1. Use `playwright-cli -s=<name> requests` (filtered to the component name) to see the actual hashed `.js` URLs the browser fetched, and `playwright-cli -s=<name> eval "fetch(url, {cache:'no-store'}).then(r => r.text())"` to confirm that specific served file's content. (The Playwright MCP server is disabled — drive the browser via the `playwright-cli` Bash CLI instead.)
