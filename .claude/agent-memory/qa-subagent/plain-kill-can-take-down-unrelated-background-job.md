...
- See also [[kill-sandbox-boundary-for-background-tasks]] for the separate, already-documented issue of plain `kill` silently failing against processes from *other* tool calls — this is a distinct failure mode (collateral death of a *survivor*, not failure to kill the *target*).
