...
- Run `eval "$(fnm env --shell bash)" && fnm use && pnpm --filter boreal-web-components exec tsc --noEmit` — expect zero type errors
- Open `bds-radio-button/types/IRadioButton.ts` and verify `RadioButtonChangeDetail` is exported alongside `IRadioButton`
- Open `bds-radio-group/types/IRadioGroup.ts` and verify the `type` union now reads `'radio' | 'radiobutton'`
...
Verify the file satisfies all rules in `.ai/guidelines/jsdoc-template.md`:

- [ ] Class-level `/** */` block describes the component and lists `@slot` for both slots (`default`, `icon`)
- [ ] Every `@Prop()` has an inline `/** */` block immediately above the decorator
- [ ] `bdsChange` `@Event()` has an inline `/** */` block; uses `@Event({ bubbles: true })` because the group catches it via `@Listen`
- [ ] No `@attr`, `@property`, `@fires`, `@element`, `@cssprop`, `@internal`, or `@part` tags
- [ ] No `@part` tags (light DOM, no shadow boundary)

**Manual test (waiveable):**

- Open `bds-radio-button.tsx` and verify no `@Prop()` is missing its JSDoc block
- Confirm `bdsChange` uses `@Event({ bubbles: true })` and has an inline `/** */`
- Confirm the class-level block has `@slot` for `default` and `icon`
...
- Run `eval "$(fnm env --shell bash)" && fnm use && pnpm --filter boreal-web-components test -- --testPathPattern="bds-radio-button"` — all three spec files must pass with zero failures
- Run `eval "$(fnm env --shell bash)" && fnm use && pnpm --filter boreal-web-components test -- --testPathPattern="bds-radio"` — existing radio and radio-group tests must remain green (no regressions)
...
- Render `<bds-radio-button label="Option A" value="a"></bds-radio-button>` in isolation (e.g. playground page or Storybook)
- Verify: pill/button border, correct padding and gap from tokens, no visible native `<input>` element rendered in the viewport
- Click → border and background color shift to the checked state (`$boreal-ui-success-base` tint)
- Hover on an unchecked button → border transitions to `$boreal-ui-success-dark`
- Render with `error` attribute → border becomes `$boreal-stroke-danger-base`
- Render with `disabled` attribute → cursor `not-allowed`, muted text and background colors, no pointer interaction
- Focus button via keyboard (Tab) → `box-shadow` focus ring appears; outline is suppressed
- Render two buttons inside `<bds-radio-group type="radiobutton">` → adjacent borders collapse correctly (segmented control shape)
...
#### Change 5f — bds-radio-button.scss: remove border-collapsing rules, divider handles separation

Replace the `bds-radio-group[type="radiobutton"]` block with:
...
bds-radio-group[type="radiobutton"] {
  ...
  bds-radio-button:first-of-type {
    border-top-left-radius: $boreal-border-radius-s;
    border-bottom-left-radius: $boreal-border-radius-s;
  }

  bds-radio-button:last-of-type {
    border-top-right-radius: $boreal-border-radius-s;
    border-bottom-right-radius: $boreal-border-radius-s;
  }
}
```

The `margin-left: -1px` border-collapsing trick is no longer needed — `bds-divider` provides the visual separation.
...
#### Fallback SCSS — same border-radius adjustments as primary (replace border-collapsing block)

Same as Change 5f in the primary approach above.
...
- Run `pnpm dev:docs` and navigate to **Forms / Radio Group** in Storybook's Stories panel
- Verify all six new stories appear in the sidebar: RadioButton, RadioButtonWithIcons, RadioButtonWithTooltips, RadioButtonNoDivider, RadioButtonDisabled, RadioButtonError
- Click buttons within each story → single selection is enforced; checked button shows correct background/border tint
- Verify RadioButtonNoDivider renders with no `bds-divider` elements between buttons
- Keyboard test inside a story: Tab into the segmented group → Arrow keys navigate and select; Space also selects; Tab exits the group
- Check the Storybook **Show code** snippet for each story — verify it reflects valid lit HTML that matches the story output
...
- Run `pnpm dev:docs` and navigate to **Forms / Radio Group → Docs** tab in Storybook
- Verify the page renders without MDX import errors in the browser console
- Verify "Radio (Circular) Variant" section appears with Canvas previews for Default, Horizontal, WithValue, WithInfoTooltip, WithIcons
- Verify "Radio Button Variant" section appears with Canvas previews for RadioButton, RadioButtonWithIcons, RadioButtonWithTooltips, RadioButtonDisabled, RadioButtonError
- Verify "States", "Form Integration", and "Accessibility" shared sections still render correctly (no content accidentally removed)
- Verify no orphaned headings, broken Canvas references, or duplicate content between the main file and variant MDX imports
