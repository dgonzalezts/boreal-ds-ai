...
- **Every `@Prop()` and `@Event()` on the component must have an `argTypes` entry, with no exceptions for "internal-only" or "JS-only" props/events.** A prop being JS-only (no HTML attribute) or an event feeling like implementation plumbing (e.g. a change event that mainly exists for framework `v-model` wiring) is not a reason to omit it — it is a reason to say so *in the description*, not to leave it out of the table. Consumers reading generated docs have no way to discover an undocumented prop/event exists at all. This was violated for `bds-table`'s `data`, `rowKey`, `selectedRows`, `searchable`, and `selectedRowsChange` (EOA-14935, caught in PR review, not before) — none had `argTypes` entries despite `selectedRows`/`searchable`/`selectedRowsChange` already being referenced in the MDX `<ArgTypes include={[...]}>` list. See `storybook-patterns.md`'s "MDX `include` completeness" note for the specific mechanism that let this stay silent.
