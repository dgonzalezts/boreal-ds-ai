...
`bds-radio` is NOT form-associated. It carries `role="radio"` and `tabindex` on the host. `select()` guards with `if (this.disabled || this.checked) return` — radios do not toggle off. Space key triggers selection. On mount, fires `bdsMount` so the group can register it.
...
1. `@Listen('bdsMount')` — guard with `LEAF_TAGS` check; stamp `name`, `checked`, `disabled`, `error` on child; set initial `tabindex`
...
export class BdsRadioGroup implements IRadioGroup {
  ...
  /** Helper text rendered below the radio options. Turns red in error state. */
  ...
  /** Disables all child radios and prevents selection. */
  @Prop({ reflect: true }) readonly disabled: boolean = false;
